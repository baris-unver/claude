...
def main():
    ...
    model, ck, _, _ = load_checkpoint(cfg.out_dir / "best.pt", dev)
    cfg.model = ck.model
    ...
    for s in samples:
        ...
        recs.append({
            ...
            "candidates": [{"lat": round(float(db.centers[j, 0]), 6), "lon": round(float(db.centers[j, 1]), 6),
                            "score": round(float(x), 4), "weight": round(float(v), 4)}
                           for j, x, v in zip(res.top_cells[0][:20], sc[:20], w[:20])],
        })
